...
function fit!{R, Rid, Rtime}(::Type{Val{R}},
    ...
    for r in 1:rank
        ...
        g! = (x, out) -> sum_of_squares_gradient!(x, out, sqrtw, res, timef.refs, idf.refs, N, lambda, invlen)
        fg! = (x, out) -> sum_of_squares_and_gradient!(x, out, sqrtw, res, timef.refs, idf.refs, N, lambda, invlen)
...
# gradient
function sum_of_squares_gradient!{Ttime, Tid}(x::Vector{Float64}, out::Vector{Float64}, sqrtw::AbstractVector{Float64}, y::Vector{Float64}, timerefs::Vector{Ttime}, idrefs::Vector{Tid}, l::Integer, lambda::Real, invlen::Real)
...
function sum_of_squares_and_gradient!{Ttime, Tid}(x::Vector{Float64}, out::Vector{Float64}, sqrtw::AbstractVector{Float64}, y::Vector{Float64}, timerefs::Vector{Ttime}, idrefs::Vector{Tid}, l::Integer, lambda::Real, invlen::Real)
    fill!(out, zero(Float64))
    len_y = length(y)
    sum = zero(Float64)
    @inbounds @simd for i in 1:length(y)
...
function sum_of_squares_hessian!{Ttime, Tid}(x::Vector{Float64}, out::Matrix{Float64}, sqrtw::Vector{Float64}, y::Vector{Float64}, timesrefs::Vector{Ttime}, idsrefs::Vector{Tid}, l::Integer, lambda::Real)
    fill!(out, zero(Float64))
    @inbounds @simd for i in 1:length(y)
        id = idsrefs[i]
        time = timesrefs[i]+l
        ...
        wi = sqrtwi^2
        error = y[i] - sqrtwi * loading * factor
        out[id, id] += 2.0 * wi * factor^2
        out[time, time] += 2.0 * wi * loading^2
...
function fit!{R, Rid, Rtime}(::Type{Val{R}}, 
    ...
    g! = (x, out) -> sum_of_squares_gradient!(x, out, sqrtw, y, timerefs, idrefs, n_regressors, rank, Xt, lambda, invlen)
    fg! = (x, out) -> sum_of_squares_and_gradient!(x, out, sqrtw, y, timerefs, idrefs, n_regressors, rank, Xt, lambda, invlen)
    d = DifferentiableFunction(f, g!, fg!)
...
function sum_of_squares_and_gradient!{Tid, Ttime}(x::Vector{Float64}, out::Vector{Float64}, sqrtw::AbstractVector{Float64}, y::Vector{Float64}, timerefs::Vector{Ttime}, idrefs::Vector{Tid}, n_regressors::Integer, rank::Integer, Xt::Matrix{Float64}, lambda::Real, invlen::Real)
    fill!(out, zero(Float64))
    sum = zero(Float64)
    # residual term
    @inbounds @simd for i in 1:length(y)
