...
for method in [:svd, :gs, :bfgs, :l_bfgs, :gradient_descent]
	precision = 1e-1
	println(method)
	result = fit(SparseFactorModel(:pState, :pYear, 1), :Sales, df , method =  method) ;
	...
	result = fit(SparseFactorModel(:pState, :pYear, 2), :Sales,  df, method =  method) ;
	@test_approx_eq_eps abs(result.augmentdf[:factors1][1])  0.18662770198472406 precision
	...
	if method != :gradient_descent
		result = fit(SparseFactorModel(:pState, :pYear, 1), Sales ~ 1 |> pState, df, method = method) ;
		...
		result = fit(SparseFactorModel(:pState, :pYear, 2), Sales ~ 1 |> pState, df, method =  method) ;
	...
	# subset
	result = fit(SparseFactorModel(:pState, :pYear, 2), Sales ~ 1 |> pState, df, method =  method, subset = (df[:State] .<= 30)) ;
