...
for method in [:dogleg, :levenberg_marquardt, :gauss_seidel]
	println(method)
	model = @model Sales ~ Price + ife(State, Year, 1)
	...
	show(result)
	model = @model Sales ~ Price + ife(State, Year, 2)
	result = regife(df, model, method = method, save = true)
	...
	model = @model Sales ~ Price + ife(State, Year, 2) + fe(State) + fe(Year)
	result = regife(df, model, method = method, save = true)
	...
	# subset
	model = @model Sales ~ Price + ife(State, Year, 2) + fe(State) subset = (State .<= 30)
	result = regife(df, model, method = method, save = true)
...
@test_throws ErrorException regife(df, @model(Sales ~ Price + ife(State, Year, 2) + fe(State2)))
...
for method in [:levenberg_marquardt, :dogleg]
	...
	model = @model Sales ~ Price + ife(State, Year, 1) + fe(State)
	...
	model = @model y ~ x1 + ife(pid1, pid2, 1)
	result = regife(df, model, weights = :w, method = method, save = true)
