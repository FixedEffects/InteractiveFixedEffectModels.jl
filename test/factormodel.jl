...
for method in [:gauss_seidel,  :dogleg, :levenberg_marquardt]
	println(method)
	result = @reg df Sales ~ 0 ife = (pState + pYear, 1) method =  method maxiter = 10_000
	...
	result = @reg df Sales ~ 0 ife = (pState + pYear, 2) method =  method maxiter = 10_000
	@test abs(result.augmentdf[:factors1][1])  ≈ 0.18662770198472406 atol = 1e-1
	@test abs(result.augmentdf[:loadings1][1])  ≈ 587.227 atol = 1e-1
	@test result.augmentdf[:residuals][1] ≈ 2.16611 atol = 1e-1

	result = @reg df Sales ~ 0 ife = (pState + pYear, 1) fe = pState method = method maxiter = 10_000
	...
	result = @reg df Sales ~ 0 ife = (pState + pYear, 2) fe = pState method =  method maxiter = 10_000
	...
	result = @reg df Sales ~ 0 ife = (pState + pYear, 2) fe = pState method =  method subset = (df[:State] .<= 30) maxiter = 10_000
