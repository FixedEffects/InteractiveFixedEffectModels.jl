...
for method in [:gauss_seidel,  :dogleg, :levenberg_marquardt]
	...
	model = @model Sales ~ ife(pState, pYear, 1) + fe(pState)
	result = regife(df, model, method = method, maxiter = 10_000)
	...
	model = @model Sales ~ ife(pState, pYear, 2) + fe(pState)
	result = regife(df, model, method = method, maxiter = 10_000)
	...
	model = @model Sales ~ ife(pState, pYear, 2) + fe(pState) subset = (State .<= 30)
	result = regife(df, model, method = method, maxiter = 10_000)
